...
export async function getPostById(postId: number): Promise<PostModel> {
  return postRepo.getPost(postId).then(toPostModel);
}
...
async function toPostModel(post: Post): Promise<PostModel> {
  const likedBy = await getByPostInteraction(post.id, "LIKE", 1, 100);
  const dislikedBy = await getByPostInteraction(post.id, "DISLIKE", 1, 100);
  return {
    id: post.id,
